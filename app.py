...
# Load the model and doctor dataset
svc = pickle.load(open("model/svc.pkl", 'rb'))
doctors_df = pd.read_csv("data/Doctor_data_with_diseases.csv")
...
symptoms_dict = {'itching': 0, 'skin_rash': 1, 'nodal_skin_eruptions': 2, 'continuous_sneezing': 3, 
                 ...
                 'indigestion': 30, 'headache': 31, 'yellowish_skin': 32, 'dark_urine': 33, 
...
diseases_list = {15:'Fungal infection', 4: 'Allergy', 16: 'GERD', 9: 'Chronic cholestasis', 14: 
                    ...
                    'Paralysis (brain hemorrhage)', 28: 'Jaundice', 29: 'Malaria', 8: 'Chicken pox', 11: 
                    'Dengue', 37: 'Typhoid', 40: 'hepatitis A', 19: 'Hepatitis B', 20: 'Hepatitis C', 21: 
                    'Hepatitis D', 22: 'Hepatitis E', 3: 'Alcoholic hepatitis', 36: 'Tuberculosis', 10: 
                    'Common Cold', 34: 'Pneumonia', 13: 'Dimorphic hemmorhoids(piles)', 18: 
                    'Heart attack', 39: 'Varicose veins', 26: 'Hypothyroidism', 24: 
                    'Hyperthyroidism', 25: 'Hypoglycemia', 31: 'Osteoarthristis', 5: 
                    'Arthritis', 0: '(vertigo) Paroymsal  Positional Vertigo', 2:
                    'Acne', 38: 'Urinary tract infection', 35: 'Psoriasis', 27: 'Impetigo'
                    }
...
# Function to suggest doctors
def suggest_doctors(disease, sort_by="both"):
...
def index():
    if request.method == 'POST':
        ...
        sort_by = request.form.get('sort_by', 'both')
        
        if not symptoms:
            return render_template('index.html', messages=["Please enter symptoms."])
        
        user_symptoms = [sym.strip() for sym in symptoms.lower().split(',')]
        predicted_disease = get_disease(user_symptoms)
        
        suggested_doctors = suggest_doctors(predicted_disease, sort_by)
        
        if isinstance(suggested_doctors, str):
